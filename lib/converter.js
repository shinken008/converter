...
function transform(target, config) {
    ...
    let nextValue = index_1.deepClone(target); // use deep clone
    switch (targetType) {
        case 'Object':
            Object.keys(target).forEach(key => {
                nextValue[key] = convertWithType(target[key], config[key].type, config[key].required, config[key].message || `${key} is required`);
            });
            break;
        case 'Array':
